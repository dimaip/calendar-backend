...
const parseSaints = html => {
    return sanitizeHtml(html.replace(/<b>/g, '<strong>').replace(/<\/b>/g, '</strong>'), {
    ...
    return zachala.then(zachalaResolver => {
        return FS.read(path.join('raw', dateString))
            .then(value => {
                ...
                readings = readings.replace(/<div class='DAYS_snoska' style='display:none'>(.*?)<\/div>/g, function(
                    str,
                    innerText
                ) {
                    innerText = sanitizeHtml(innerText, {
                ...
                readings
                    .replace(/\s*((?:Утр\.|Лит\.))\s*-\s*/g, '|$1@')
                    .replace(/\s*((?:На веч\.|На \d-м часе))\s*[:]\s*/g, '|$1@')
                    .split('|')
                    .map(item => {
                        ...
                        val.replace(/Ев.\s\d-е,/g, '')
                            .replace(/\s*((Прп\.|[а-яА-Я0-9 .-]+))\s*[:]\s*/g, '|$1@')
                            .split('|')
                            .map(item => {
                                ...
                                if (subVal)
                                    subValues[subKeyMap[subKey] || subKey] = createReadingLinks(
                                        subVal,
                                        zachalaResolver
                                    );
                            });

                        if (Object.keys(subValues).length > 0) values[keyMap[key]] = subValues;
                    });
            ...
            .then(data => {
                ...
                return {
                    ...
                    fast: $('.DD_POST')
                        ...
                        .trim()
                        .replace(/\.$/, ''),
                    fast_detail: $('.DD_TPTXT')
                        .text()
                        .trim()
                        .replace(/\.$/, ''),
                    originalTitle: $('.DP_TEXT.DPN_-1 .DNAME a')
                        .text()
                        .trim()
                        .replace(/\.$/, ''),
            ...
            .then(value =>
                FS.write(path.join(processedPath, dateString), value).catch(e =>
                    console.log("can't write", e, dateString)
                )
            )
